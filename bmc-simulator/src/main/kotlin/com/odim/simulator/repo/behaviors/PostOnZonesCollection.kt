...
import com.odim.simulator.tree.ResourceLinker.areLinked
...
class PostOnZonesCollection : Behavior {
    override fun run(tree: ResourceTree, item: Item, request: Request, response: Response, dataStore: BehaviorDataStore): BehaviorResponse {
        val zonesCollection = item as ResourceCollection
        val zone = tree.create(zonesCollection.type.of())
        zonesCollection.add(zone)

        return terminal(try {
            Merger.merge(tree, zone, request)
            val endpointList = request.json?.let { gatherLinkedResources(it, tree) }

            endpointList?.forEach {
                if (!areLinked(it, zone)) {
                    tree.link(it, zone)
                }
            }
            created(zone)
    ...
    private fun gatherLinkedResources(json: ObjectNode, tree: ResourceTree): List<Resource> {
        val endpointsJsons = json.getArray("Links/Endpoints")
        return endpointsJsons.mapNotNull { tree.search((it as ObjectNode).getString("@odata.id")) as Resource }
    }
}
