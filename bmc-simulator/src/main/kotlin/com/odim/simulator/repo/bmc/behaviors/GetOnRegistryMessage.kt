...
class GetOnRegistryMessage : Behavior {
    override fun run(tree: ResourceTree, item: Item, request: Request, response: Response, dataStore: BehaviorDataStore): BehaviorResponse {
        updateRegistryMessage(item as Resource)
        return nonTerminal(success(item))
    }

    private fun updateRegistryMessage(registry: Resource) {
        registry {
            "Languages@odata.count" to registry.traverse<EmbeddedArray<Any?>>("Languages").size
            "Location@odata.count" to registry.traverse<EmbeddedArray<Any?>>("Location").size
        }
    }
}
