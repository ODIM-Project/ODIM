...
import com.odim.simulator.tree.structure.ResourceCollectionType.POWER_DOMAINS_COLLECTION
 ...
 */
@Template(FACILITY)
open class FacilityTemplate : ResourceTemplate() {
    init {
        version(V1_0_0, resourceObject(
                ...
                "Name" to "Facility",
                "FacilityType" to "Room",
                "Status" to embeddedObject(STATUS),
                "Location" to embeddedObject(RESOURCE_LOCATION),
                "PowerDomains" to LinkableResource(POWER_DOMAINS_COLLECTION),
                "Actions" to Actions(),
                "Links" to embeddedObject(
                        "Oem" to embeddedObject(),
                        "ContainedByFacility" to LinkableResource(FACILITY),
                        "ContainsFacilities" to LinkableResourceArray(FACILITY),
                        "ManagedBy" to LinkableResourceArray(MANAGER),
                        "ContainsChassis" to LinkableResourceArray(CHASSIS),
                        "FloorPDUs" to LinkableResourceArray(POWER_DISTRIBUTION),
                        "RackPDUs" to LinkableResourceArray(POWER_DISTRIBUTION),
                        "TransferSwitches" to LinkableResourceArray(POWER_DISTRIBUTION),
                        "Switchgear" to LinkableResourceArray(POWER_DISTRIBUTION)
                )
        ))
