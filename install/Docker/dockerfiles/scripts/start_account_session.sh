...
start_account_session()
{
	...
	/bin/svc-account-session --registry=etcd --registry_address=${registry_address} --server_address=account-session:45101   --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s  2>& | tee /var/log/odimra_logs/account_session.log
	PID=$!
	sleep 3
