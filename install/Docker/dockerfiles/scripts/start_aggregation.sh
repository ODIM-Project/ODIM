...
start_aggregation()
{
	...
	/bin/svc-aggregation  --registry=etcd --registry_address=${registry_address} --server_address=aggregation:45102 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s  2>&1 | tee /var/log/odimra_logs/aggregation.log
	PID=$!
	sleep 3
