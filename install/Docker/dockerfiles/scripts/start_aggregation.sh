...
start_aggregation()
{
	...
	nohup /bin/svc-aggregation --registry=consul --registry_address=${registry_address} --server_address=aggregation:45102 --framework=GOMICRO --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/aggregation.log 2>&1 &
	PID=$!
	sleep 3
...
monitor_process()
{
        while true; do
                ...
                if [[ $? -ne 0 ]] || [[ $pid -gt 1 ]]; then
                        echo "[$(date)] -- ERROR -- svc-aggregation not found running, exiting"
			kill -15 ${OWN_PID}
                        exit 1
                fi
                sleep 5
