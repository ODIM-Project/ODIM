...
start_aggregation()
{
	...
	nohup /bin/svc-aggregation --framework=GRPC --registry=etcd --registry_address=${registry_address} --server_address=aggregation:45102 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/aggregation.log 2>&1 &
	PID=$!
	sleep 3
