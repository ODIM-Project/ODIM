...
start_composition_service()
{
        ...
        nohup /bin/svc-composition-service --registry=etcd --registry_address=${registry_address} --server_address=composition-service:45112 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/composition_service.log 2>&1 &
