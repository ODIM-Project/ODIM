...
start_event()
{
	...
	nohup /bin/svc-events --registry=etcd --registry_address=${registry_address} --server_address=events:45103 =GRPC --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/events.log 2>&1 &
	PID=$!
	sleep 3
