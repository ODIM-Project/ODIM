...
start_event()
{
	...
	nohup /bin/svc-events --registry=etcd --registry_address=${registry_address} --server_address=events:45103  --client_request_timeout=${client_request_timeout} >> /var/log/odimra_logs/events.log 2>&1 &
	fi
	PID=$!
	sleep 3
