...
start_fabrics()
{
	...
	nohup /bin/svc-fabrics  --registry=etcd --registry_address=${registry_address} --server_address=fabrics:45106 --client_request_timeout=${client_request_timeout} >> /var/log/odimra_logs/fabrics.log 2>&1 &
        fi
	PID=$!
	sleep 3
