...
start_fabrics()
{
	...
	/bin/svc-fabrics  --registry=etcd --registry_address=${registry_address} --server_address=fabrics:45106 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s  2>&1  | tee /var/log/odimra_logs/fabrics.log
	PID=$!
	sleep 3
