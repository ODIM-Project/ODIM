...
create_signal_trap()
{
        trap 'echo "[$(date)] -- INFO  -- SIGTERM received for licenses, initiating shut down"; sigterm_handler' SIGTERM
}
...
{
        while true; do
                pid=$(pgrep -fc svc-licenses 2> /dev/null)
                if [[ $? -ne 0 ]] || [[ $pid -gt 1 ]]; then
                        echo "[$(date)] -- ERROR -- svc-licenses not found running, exiting"
			kill -15 ${OWN_PID}
                        exit 1
                fi
                sleep 5
