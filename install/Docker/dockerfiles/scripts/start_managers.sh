...
create_signal_trap()
{
        trap 'echo "[$(date)] -- INFO  -- SIGTERM received for manager, initiating shut down"; sigterm_handler' SIGTERM
}
...
start_manager()
{
	...
	if [ $HA_ENABLED != 'true' ]; then
		...
		nohup ./svc-managers --registry=consul --registry_address=[consul1:8500,consul2:8500,consul3:8500] --server_address=managers:45107 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/managers.log 2>&1 &
	fi
	PID=$!
	sleep 2s
