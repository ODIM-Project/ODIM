...
start_systems()
{
	...
	nohup /bin/svc-systems --registry=etcd --registry_address=${registry_address} --server_address=systems:45104 --framework=GRPC  --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/systems.log 2>&1 &
	PID=$!
	sleep 3
