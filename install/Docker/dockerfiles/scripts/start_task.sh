...
start_task()
{
	...
	/bin/svc-task --registry=etcd --registry_address=${registry_address} --server_address=task:45105  --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s 2>&1 | tee /var/log/odimra_logs/task.log
	PID=$!
	sleep 3
