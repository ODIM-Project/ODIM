...
func auditLogEntry(ctx iris.Context, reqBody map[string]interface{}, sessionUserName, sessionRoleID string ) string {
    var logMsg string
...
func AuditLog(ctx iris.Context, reqBody map[string]interface{}, sessionUserName, sessionRoleID string){
    logMsg := auditLogEntry(ctx, reqBody, sessionUserName, sessionRoleID)
    // Get response code
...
// AuthLog function
func AuthLog(sessionUserName, sessionRoleID string, respStatusCode int32){
    var logMsg string
    timeNow := time.Now().Format(time.RFC3339)
    // formatting logs in syslog format
	...
	if operationStatus {
		successMsg := "<86> " + logMsg + " Authentication/Authorization successful"
		fmt.Println(successMsg)
	} else {
	    msg := " Authentication/Authorization failed"
	    if respStatusCode == http.StatusForbidden{
            msg = " Authorization failed"
        } else if respStatusCode == http.StatusUnauthorized {
            msg = " Authentication failed"
        }
		failedMsg := "<84> " + logMsg + msg
		fmt.Println(failedMsg)
	}
}