...
create_packages()
{
	packages_list=(odimra-platformconfig odimra-config configure-hosts odimra-pv-pvc consul-ha consul redis-ha redis zookeeper-ha zookeeper kafka-secret zookeeper-secret odimra-secret reloader update task systems managers fabrics telemetry events api aggregation account-session kafka-ha kafka)
