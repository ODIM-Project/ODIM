...
//ClearLog : Clear log at log service
func ClearLog(ctx iris.Context) {
	...
	if err != nil {
		...
		fmt.Println(err)
		if resp == nil {
			ctx.WriteString("Error while clear log " + errorMessage)
			ctx.StatusCode(http.StatusInternalServerError)
			return
		}
	}
	...
	if err != nil {
		errorMessage := err.Error()
		fmt.Println(err)
		ctx.WriteString("Error while trying clear log " + errorMessage)
	}
