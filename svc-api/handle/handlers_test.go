...
func TestGetMessageRegistryFileID(t *testing.T) {
	...
	err = ioutil.WriteFile("/tmp/Base.1.16.0.json", message, 0644)
	if err != nil {
		t.Fatalf(err.Error())
	}
	router := iris.New()
	...
	test.GET("/redfish/v1/Registries/Base.1.16.0").WithHeader("X-Auth-Token", "validToken").Expect().Status(http.StatusOK)
	test.GET("/redfish/v1/Registries/Base.1.16.0").Expect().Status(http.StatusUnauthorized)
	test.GET("/redfish/v1/Registries/Base.1.16.0").WithHeader("X-Auth-Token", "invalidToken").Expect().Status(http.StatusUnauthorized)
}
func TestGetMessageRegistryFile(t *testing.T) {
	...
	test := httptest.New(t, router)
	test.GET("/redfish/v1/registries/UnknownID").WithHeader("X-Auth-Token", "validToken").Expect().Status(http.StatusNotFound)
	test.GET("/redfish/v1/registries/Base.1.16.0.json").WithHeader("X-Auth-Token", "validToken").Expect().Status(http.StatusOK)
	test.GET("/redfish/v1/registries/Base.1.16.0.json").Expect().Status(http.StatusUnauthorized)
	test.GET("/redfish/v1/registries/Base.1.16.0.json").WithHeader("X-Auth-Token", "invalidToken").Expect().Status(http.StatusUnauthorized)
}
