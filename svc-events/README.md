...
Aggregator for ODIM offers an event interface that allows northbound clients to interact and receive notifications such as alerts and alarms from multiple s, including Aggregator for ODIM itself. It exposes Redfish `EventService` APIs for managing events.
...
This endpoint subscribes a northbound client to events originating from a set of s \(southbound devices, managers, Aggregator for ODIM itself\) by creating a subscription entry. For use cases, see [Subscription use cases](#event-subscription-use-cases).


It is performed as a task. If there is more than one  that is sending a specific event, the task is further divided into subasks. To know the progress of this action, perform HTTP `GET` on the [task monitor](#task-monitor) returned in the response header \(until the task is complete\).
...
|Types|Array \(string, null\)|Read-only \(Optional\)<br> |The list of  type values \(Schema names\) that correspond to the `Origins`. For possible values, perform `GET` on `redfish/v1/EventService` and check values listed under `Types` in the JSON response.<br> Examples: "ComputerSystem", "Storage", "Task"<br> |
...
|String|Description|
|------|-----------|
|A single |A specific  for which the service sends only related events.|
|A list of s. Supported collections:<br> |A collection of s for which the service will send only related events.|
|/redfish/v1/Systems|All computer system s available in Aggregator for ODIM for which the service sends only related events. By setting `EventType` property in the request payload to `Added` or `Removed` and `Origins` property to `/redfish/v1/Systems`, you can receive notifications when a system is added or removed in  Aggregator for ODIM.|
|/redfish/v1/Chassis|All chassis s available in Aggregator for ODIM for which the service sends only related events.|
|/redfish/v1/Fabrics|All fabric s available in Aggregator for ODIM for which the service sends only related events.|
|/redfish/v1/TaskService/Tasks|All tasks scheduled by or being executed by Redfish `TaskService`. By subscribing to Redfish tasks, you can receive task status change notifications on the subscribed destination client.<br> By specifying the task URIs as `Origins` and `EventTypes` as `StatusChange`, you can receive notifications automatically when the tasks are complete.<br> To check the status of a specific task manually, perform HTTP `GET` on its task monitor until the task is complete.<br> |
...
|Added|A  has been added.|
|Removed|A  has been removed.|
|Updated|The value of this  has been updated.|
|StatusChange|The status of this  has changed.|

### EventFormat type

|String|Description|
|------|-----------|
|Event|The subscription destination will receive JSON bodies of the  Type Event.|
...
|Types|Array \(string, null\)|Read-only  <br> |The list of  type values \(Schema names\) that correspond to `OriginOfCondition`. For possible values, perform `GET` on `redfish/v1/EventService` and check values listed under `Types` in the JSON response.<br> Examples: "ComputerSystem", "Storage", "Task"<br> |
...
|OriginOfCondition|String|Optional|The URL in the `OriginOfCondition` property of the event to add. It is not a reference object. It is the  that originated the condition that caused the event to be generated. For possible values, see "Origin s" in [Creating an event subscription](#creating-an-event-subscription).|
...
|**Returns** |JSON schema having the details of this subscription–subscription Id, destination, event types, origin , and so on.|
|**Response code** |`200 OK` |
|**Authentication** |Yes|
...
```
{
   ...
   "Message":"The  has been removed successfully.",
   "MessageId":"Event.1.0.2.Removed",
   "Severity":"OK"
}
```
