...
import (
	...
	"github.com/ODIM-Project/ODIM/svc-license/rpc"
...
func main() {
	if uid := os.Geteuid(); uid == 0 {
		log.Error("License Service should not be run as the root user")
	}
...
func registerHandlers() {
	if err := services.InitializeService(services.License); err != nil {
		log.Error("fatal: error while trying to initialize service: " + err.Error())
	}
	license := rpc.GetLicense()
	licenseproto.RegisterLicenseServer(services.ODIMService.Server(), license)
}
