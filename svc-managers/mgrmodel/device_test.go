...
func TestUpdateSystem(t *testing.T) {
	...
	mockSystemData("test123", device)

	err := UpdateSystem("test123", device)
	assert.Nil(t, err, "error should be nil ")
	err = UpdateSystem("test1234", device)
	assert.NotNil(t, err, "error should not be nil ")
}

func mockSystemData(deviceUUID string, device *DeviceTarget) {
	conn, err := common.GetDBConnection(common.OnDisk)
	if err != nil {
		return
	}
	err = conn.Create("System", deviceUUID, device)
	if err != nil {
		return
	}
}

func TestAddTempPassword(t *testing.T) {
	config.SetUpMockConfig(t)
	defer func() {
		err := common.TruncateDB(common.OnDisk)
		if err != nil {
	...
	// mockTempPassword("test", device)
	err := AddTempPassword("test", device)
	assert.Nil(t, err, "error should be nil")
	err = AddTempPassword("test", device)
	assert.NotNil(t, err, "error should not be nil")
}

func mockTempPassword(deviceUUID string, device *DeviceTarget) {
	conn, err := common.GetDBConnection(common.OnDisk)
	if err != nil {
		return
	}
	err = conn.Create("tempBmcPassword", deviceUUID, device)
	if err != nil {
		return
	}
}

func TestGetTempPassword(t *testing.T) {
	config.SetUpMockConfig(t)
	defer func() {
		err := common.TruncateDB(common.OnDisk)
		if err != nil {
	...
	_, err := GetTempPassword("test")
	assert.Nil(t, err, "err should be nil")

	_, err = GetTempPassword("invalid")
	assert.NotNil(t, err, "err should not be nil")

}

func TestDeleteTempPassword(t *testing.T) {
	config.SetUpMockConfig(t)
	defer func() {
		err := common.TruncateDB(common.OnDisk)
		if err != nil {
